...
- **位置**：Phase 1 · M0 世界层，推进至 **M0-11a**（D4 变换段池：2048 段×16 槽 + 相位 4 游标
  执行器 + 12 op）；main 全绿（单测/clippy/金向量/烘焙表）。
- **在飞**：无。
- **下一步候选**：M0-11b（`WAIT_SIGNAL`/`BOUNCE_ARM`/STEP 插值）· bomb（`FieldPool` 首个真租户，
  闭合自机能力）· 道具池 · 敌人 move_to 插值器。
- **待办**：技术债见 [`docs/follow-ups.md`](docs/follow-ups.md)（开工前先读）。
