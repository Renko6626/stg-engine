...
- **位置**：**敌句柄打包刀**（`feat/enemy-handle-gen`）——脚本敌号从**裸池索引**改成
  `((gen & 0x7FFF) << 16) | index`，堵掉槽复用后旧敌号静默指向另一只敌的 ABA。
  **不是加机制，是把已有的信息接上**：`EnemyHandle` 本来就带 generation，只是 syscall
  边界丢了它。零新 syscall 号、不动 `World` 布局；`ENGINE_VER` 8→9（**取值编码变更**，
  比号表新增更硬）。金向量与 7a6945d 对拍**逐字节不变**，两个冒烟真跑绿。
- **在飞**：无。
...
- **待办**：见 [`docs/follow-ups.md`](docs/follow-ups.md)（新记 **D15**：敌号只带 gen 低
