...
- **位置**：**敌坐标读口刀**（`feat/enemy-pos-read`）——补上一刀暴露的断头路：syscall
  80/81（`enemy_x`/`enemy_y`）按敌号读坐标，`nearest_enemy → 探活 → 读坐标 → atan2 →
  fire` 这条链路接通（一条 `.ecl` 源码级 e2e 押着）。零新机制、不动 `World` 布局；
  `ENGINE_VER` 6→7（纯号表理由）。金向量与 33b2c4d 对拍**逐字节不变**。
- **在飞**：无。
...
- **待办**：见 [`docs/follow-ups.md`](docs/follow-ups.md)（本刀销 C12⑤ 里上一刀追记的读口
  缺口；该条余下的 `last_status`/`attract_all_items` 仍是候补）。
