...
- **位置**：Phase 1 · M0 世界层，推进至 **M0-14**（`create_bullets_batch` N×K 网格发射器——
  环/列/多重环一个原语，M1 ECL 要调的世界 API 面就绪）；main 全绿（单测/clippy/金向量/烘焙表）。
- **在飞**：无。
- **下一步候选**：bomb · `SPAWN_PATTERN` + 图样表 · M1 ECL。
