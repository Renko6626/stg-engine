...
- **位置**：Phase 1 · M0 世界层，推进至 **M0-11b**（D4 变换系统全量落地：信号黑板 + 场界反弹 +
  STEP 缓动，十六 op 齐装）；main 全绿（单测/clippy/金向量/烘焙表）。
- **在飞**：无。
- **下一步候选**：bomb（`FieldPool` 首个真租户，闭合自机能力）· 道具池 · 敌人 move_to 插值器 ·
  `SPAWN_PATTERN` + 图样表。
- **待办**：技术债见 [`docs/follow-ups.md`](docs/follow-ups.md)（开工前先读；B1 池满降级零测试仍是最值一刀）。
