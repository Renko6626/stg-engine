...
- **位置**：**P4 覆盖刀**（宪法级不变量 P4-a/P4-b 的零覆盖分支补齐 + 两处真缺陷修复）
  ——四池满降级（B1）/`push_event` 溢出（B2）/overkill hp 断言（B4）/越界 `drop_table`
  （B11）/task 号支路①③两族（B25）五条补测试；`credit_item` u8 饱和（B12）+
  `storm --saves 0` 假绿（B14）两处真缺陷连测试一并修；销 follow-ups 七条。
- **在飞**：无。
