...
- **位置**：Phase 1 · M0 世界层，推进至 **M0-10**（D3 双表示运动模型：POLAR/CART 模式位 +
  九个公开 setter 写 API + 1/16 px/帧阈值回填契约）；main 全绿（单测/clippy/金向量/烘焙表）。
- **在飞**：无。
- **下一步候选**：D4 变换系统（需求讨论中）· bomb（`FieldPool` 首个真租户，闭合自机能力）·
  道具池 · 敌人 move_to 插值器。
