...
- **位置**：**真美术接入 + 命中特效通道**（M2 之后的内容/表现层连续小刀）——原作弹片切成
  12 弹型 × 16 色图集、词表按行序登记、色名三轮测量定稿；弹的渲染朝向补四分之一圈
  （贴图头朝上 vs BAM 0 指右）；自机弹命中走 `events` 通道，桥面补 `frame_events()` 读口
  （**一次性把八种事件全对宿主开放**，此前宿主只能从通道 B 看到敌死）。
  其下是**弹幕颜色轴刀**（12×16 identity 表 / ECL 两参糖 / 编译期三判据 / 部分设两 op）。
- **在飞**：无。
- **画面已目验**（2026-07-27，作者有头环境）：真美术上屏正常 → **B23 UV 镜像嫌疑已销**
  （shader 不改）。B26 三件套余 ②`visible_instances` 断言 + ③ 可玩性（打到结算/手感/节奏）。
