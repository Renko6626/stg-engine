...
- **位置**：**探活读口刀**（`feat/enemy-alive`）——堵上一刀留下的残余缝：syscall 82
  `enemy_alive` 专用探活口（旧探针 `enemy_hp(e) != -1` 在"活敌血量恰为 −1"那一格误判）。
  判据逐字同读族，**含 dying**（判「槽有效」不判「还能打」，人类裁定）。零新机制、不动
  `World` 布局；`ENGINE_VER` 7→8（纯号表理由）。金向量与 d137ecd 对拍**逐字节不变**。
- **在飞**：无。
...
- **待办**：见 [`docs/follow-ups.md`](docs/follow-ups.md)（C12⑤ 余下的 `last_status`/
  `attract_all_items` 仍是候补；本刀未新增债）。
