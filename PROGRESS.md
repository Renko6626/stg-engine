...
- **位置**：**敌人运动动词族刀**（`feat/enemy-motion-verbs`，六 Task 全落地）——敌人此前
  只有 `move_to` 一条位置动词，`vx/vy` 那条积分分支自建成起没有任何 syscall 能写、永远在
  加零。这刀补齐速度层：敌池 `speed/angle` 双表示 + 速度插值器（+12 字段/30 B 每敌），四条
  动词 `move_vel`/`move_vel_xy`/`move_angle`/`move_speed`（syscall 83-86）与四个 `$self_*`
  读口（87-90，白名单 8→12），积分相分层仲裁（速度插值恒跑 / 位置归 `mv_active`）。招牌
  是**两条插值空间不同**（极坐标匀速扫弧 vs 笛卡尔直线穿过），以及到点清速改由黏滞位
  `vel_touched` 判定（速度插值常先到期，拿 `vel_active` 当判据会误清）。`ENGINE_VER` 9→10
  （**理由是池布局**，不是号表）；金向量**预期变化**并已重生成（两段场景 frame 0 起全差）。
- **在飞**：无。
