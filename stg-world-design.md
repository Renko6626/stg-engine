...
**一次声明、处处一致**——池纪律的每一条都是"加字段时容易忘"型风险，唯一可靠解法是单一真相源自动生成：
...
2. **句柄类型**：`{ index: u16, generation: u16 }` 打包 u32；槽复用 generation+1；`NULL = index 0xFFFF`；
3. **分配/释放**：LIFO free-list（确定）+ 存活位掩码（`[u64; CAP/64]`，升序遍历）；
4. **`XxxInit` 全字段初始化结构体**：分配必须整个传入——**"写满槽"从运行期断言升级为编译期事实**（漏字段编译不过）。debug 构建保留复用槽校验断言作二线防御；
5. **迭代器**（升序 alive 遍历）与 **PoolView**（通道 A）；
