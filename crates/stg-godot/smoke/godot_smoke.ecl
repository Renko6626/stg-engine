...
sub main() {
    ...
    loop {
        ...
        // 表现契约 v2:上面那颗在 y=-160 出生当帧就越界回收(弹回收线 y ≥ -64),从没活过——
        // 弹层上传链判别要一颗**场内**的:从 (-150,300) 以 0.5 朝右飞,frame 120 时 x=-90.5、
        // y 恒 300(远离自机 y=384 与靶敌 y=200,不碰任何判定)。每轮先发的那颗占槽 0 又当帧
        // 释放,这颗落槽 1/2/3/4——压实后实例 0 = frame 1 发的这颗。
        _ = fire(BALL, COLOR_CYAN, -150.0fx, 300.0fx, 0.5fx, 0deg, none, none);
