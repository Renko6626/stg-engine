...
///
/// **存在的理由是跨 crate**（`dispatch` 是 `pub(crate)`、76 条结构测试的表是 `cfg(test)`，
...
/// 断言"全 `u16` 域里为真的号恰好是那 76 条"，漏一条/多一条即红。
...
mod tests {
    ...
    /// 【本刀的主判据】号表族结构：76 条、无重号、每条落在其声明族的百位区间内
    /// （原 74 条 + 本批新增 `513 add_time_stops`/`560 time_stop_player` = 76）。
    ///
    ...
    fn syscall_table_is_hundred_partitioned_and_unique() {
        ...
        assert_eq!(
            table.len(),
            76,
            "74 + 本批两条新 syscall（513/560）= 76：增改需同步这个数"
        );
    ...
    fn ecl_ops_doc_syscall_numbers_match_the_constants() {
        ...
        assert!(
            doc_pairs.len() >= 76,
            "只从 ecl-ops.md 解析出 {} 个 (号,名) 对，远少于 76——解析器多半没跟上排版变更",
            doc_pairs.len()
        );
