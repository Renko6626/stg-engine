...
pub struct Task {
    ...
    pub pc: u32,
    /// 剩余**要被跳过的帧数**；>0 时调度层门禁跳过本任务并递减（T2 接线）。
    ///
    /// ⚠️ 这个值是 `wait(n)` 的 **n − 1**，不是 n——`OP_WAIT` 的 yield 本身已经吃掉了当前帧
    /// 的剩余部分，那一帧不需要再由本计数器来买。于是 `wait(n)` 的周期恰是 n（2026-08-01
    /// 语义修正；此前存 n，周期是 n+1，凡自己记帧数的脚本一律偏 1/n）。`n == 0` 根本不写
    /// 本字段——那是同帧继续的真 no-op。
    pub wait: u16,
