...
"#;
    ...
    ///
    /// ⚠️ **本数组里钉死的 `60, N` 是 `OP_SYS`（60）+ 其操作数**——`N` 就是
    /// `SYS_CREATE_BULLET`/`SYS_CREATE_BULLETS_BATCH` 的取值本身，字面编码进字节流。
    /// syscall 号表重排（2026-07-31 百分区，`syscall.rs`）时，这两个数组必须**同步更新**
    /// 成新号（`SYS_CREATE_BULLET` 20→200、`SYS_CREATE_BULLETS_BATCH` 21→201）——这是
    /// **预期变更**（号表重排的定义就是"同一个号在新旧两版指向不同 syscall"，编码进字节
    /// 流的操作数必然跟着变），不是本测试逮到的 codegen 回归。判据本身没变（仍然逐字节
    /// 钉死"折叠下标改动前"的产物）；变的只是被钉的那个正确值需要重新对齐成"本刀之后"的
    /// 正确产物。**除了这两个操作数外，数组里任何其他字节的变化才是真回归**——已用
    /// byte-diff 核实过，见 task-1-report.md。
    #[test]
    fn fire_and_batch_bytecode_is_byte_for_byte_unchanged() {
        ...
        assert_eq!(
            ...
            &[
                ...
                200, 14, 10, 3, 12, 0, 10, 16, 11, 0, 20, 10, 65536, 10, 131072, 10, 196608, 10, 0,
                10, 0, 10, 0, 10, 4294967295, 60, 200, 14, 0
            ],
            "fire 的产物必须与折叠下标改动前逐字节相同（除 syscall 号重排换的操作数外）"
        );
        ...
        assert_eq!(
            batch.code(),
            &[
                10, 19, 10, 0, 10, 0, 10, 1, 10, 0, 10, 0, 10, 1, 10, 65536, 10, 0, 60, 201, 14,
                10, 3, 12, 0, 10, 16, 11, 0, 20, 10, 0, 10, 0, 10, 1, 10, 0, 10, 0, 10, 1, 10,
                65536, 10, 0, 60, 201, 14, 0
            ],
            "batch 的产物必须与折叠下标改动前逐字节相同（除 syscall 号重排换的操作数外）"
        );
    }
