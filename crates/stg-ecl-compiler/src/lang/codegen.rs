...
mod tests {
    ...
    /// 6 = 无位置插值 ⇒ `pos += vel`，y +3 → 183；
    /// 7 = `on_enemy` 的 `wait(5)` 醒来（第 2 帧执行 `wait(5)` ⇒ 计数器 4 ⇒ 跳过 3/4/5/6、
    ///     第 **2+5=7** 帧接着跑；相位 2 早于相位 5），故脚本读到的是 **183** / 3.0；
    ///     本帧与第 8 帧的相位 5 再各走一步 → `e.y()` 收在 189。
    ///
    /// ⚠️ 这段推导在 `wait` 语义修正刀（2026-08-01）之后重算过：旧内核 `wait(n)` 的周期是
    /// n+1，醒来点在第 8 帧、读到 186。**改的是脚本的醒来点，不是世界的时间线**——同一测试
    /// 里 `e.y()` 仍是 189、一帧没动，正是这一点把"语义变了"与"世界演化变了"区分开。
    #[test]
