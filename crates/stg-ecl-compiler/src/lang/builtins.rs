...
const BUILTINS: &[Builtin] = &[
    ...
    // ── 表现契约 v2（2026-09-07）：敌人表现状态号（syscall 430）────────────────
    Builtin {
        name: "set_anm_state",
        syscall: syscall::SYS_SET_ANM_STATE,
        is_op: false,
        params: &[Val(Int)],
        ret: None,
    ...
    // ── 表现契约 v2（2026-09-07）：一次性演出两条（syscall 721/722，钉死布局的 emit_req）──
    Builtin {
        name: "fx_at",
        syscall: syscall::SYS_FX_AT,
        is_op: false,
        params: &[Val(Fx), Val(Fx), Val(Int), Val(Int)],
        ret: None,
        doc: "在 (x,y) 起一次性演出:发 REQ_FX_AT [x raw,y raw,kind,param,0,0](对应 ZUN anmPlayPos);kind/param 引擎不解释,归内容包与壳侧约定;owner 无限制;即发即忘",
        param_names: &["x", "y", "kind", "param"],
    },
    Builtin {
        name: "stage_clear",
        syscall: syscall::SYS_STAGE_CLEAR,
        is_op: false,
        params: &[Val(Int)],
        ret: None,
        doc: "本关结束:发 EVT_STAGE_CLEARED{stage} 事实事件并**让出一帧**(降低成 SYS 723 + wait(1))——宿主停拍画结算页,下一条语句在放行后的第一帧才执行;先 add_score 再调它",
        param_names: &["stage"],
    },
    Builtin {
        name: "fx_on",
        syscall: syscall::SYS_FX_ON,
        is_op: false,
        params: &[Val(Int), Val(Int)],
        ret: None,
        doc: "在敌自身上起依附演出(self owner 非 ENEMY → Fault):发 REQ_FX_ATTACHED [index,gen,kind,param,0,0](对应 ZUN anmPlay);壳侧按句柄每帧跟随、句柄失效即自毁;即发即忘",
        param_names: &["kind", "param"],
    },
    // ── 读/杂项 ─────────────────────────────────────────────────────────
    Builtin {
        name: "rand",
    ...
    Builtin {
        ...
        params: &[Val(Int)],
        ret: None,
        doc: "增减 X 键库存(時環晷机体=停止,经典机体=bomb):delta 允许负,双边钳 [0,STOP_STOCK_MAX=5] 不回绕;开局初值走 Loadout,故只有 add_ 没有 set_",
        param_names: &["delta"],
    },
    Builtin {
        name: "add_power",
    ...
    // ── 8xx 激光族（激光池刀 Task 4，2026-09-25；spec 2026-09-25-laser-pool-design §5）──
    //    `laser()` 建一条并返回打包句柄（编码同敌号），其余 `lz_*` 拿这个句柄操作；
    //    有返回值的只有 `laser` 与 `lz_alive`。号表/降级口径见 `docs/ecl-ops.md` 8xx 族。
    Builtin {
        name: "laser",
        syscall: syscall::SYS_LASER_CREATE,
        is_op: false,
        params: &[
            Val(Int),
            Val(Fx),
        ...
        ret: Some(Int),
        doc: "建一条直线激光:color 0..15(越界 Fault);(x,y) 原点、angle 方向、len 长度、width 判定宽度(画多宽判多宽)、warn/active/fade 三段时长(帧);返打包激光句柄(不透明,别猜数值;失败 -1)。形态一固定 start=0、end=len、speed=0",
        param_names: &[
            "color", "x", "y", "angle", "len", "width", "warn", "active", "fade",
        ],
    },
    Builtin {
        name: "lz_speed",
        syscall: syscall::SYS_LASER_SPEED,
        is_op: false,
        params: &[Val(Int), Val(Fx), Val(Fx)],
        ret: None,
        doc: "形态二:设激光沿射线速度 speed 与棒长 start_len,并从近端长出去(end=start);两值世界层钳 [0,LASER_LEN_MAX]",
        param_names: &["lz", "speed", "start_len"],
    },
    Builtin {
        name: "lz_start",
        syscall: syscall::SYS_LASER_START,
        is_op: false,
        params: &[Val(Int), Val(Fx)],
        ret: None,
        doc: "激光近端留空 s(原作第 4 关 =64):start=s,start>end 时 end 抬到 start;世界层钳 [0,LASER_LEN_MAX]",
        param_names: &["lz", "s"],
    },
    Builtin {
        name: "lz_omega",
        syscall: syscall::SYS_LASER_OMEGA,
        is_op: false,
        params: &[Val(Int), Val(Angle)],
        ret: None,
    ...
    Builtin {
        name: "lz_rotate",
        syscall: syscall::SYS_LASER_ROTATE,
        is_op: false,
        params: &[Val(Int), Val(Angle)],
        ret: None,
        doc: "激光一次性转 a(回绕加,只作用当帧;原作 88)",
        param_names: &["lz", "a"],
    },
    Builtin {
        name: "lz_aim",
        syscall: syscall::SYS_LASER_AIM,
        is_op: false,
        params: &[Val(Int), Val(Angle)],
        ret: None,
        doc: "激光指向自机 0 再加偏移 off(原作 89);自机狙写 lz_aim(lz, 0deg),相对自机角写 laser(..., aim_player() + a, ...)",
        param_names: &["lz", "off"],
    },
    Builtin {
        name: "lz_anchor",
        syscall: syscall::SYS_LASER_ANCHOR,
        is_op: false,
        params: &[Val(Int), Val(Int), Val(Fx), Val(Fx)],
        ret: None,
        doc: "把激光挂到敌号 enemy 上并带偏移 (ox,oy)(每帧跟随;enemy=-1 解除)。失效敌号(死了/槽被复用,且非 -1)不挂靠+计数;偏移世界层钳 ±LASER_COORD_MAX",
        param_names: &["lz", "enemy", "ox", "oy"],
    },
    Builtin {
        name: "lz_origin",
        syscall: syscall::SYS_LASER_ORIGIN,
        is_op: false,
        params: &[Val(Int), Val(Fx), Val(Fx)],
        ret: None,
        doc: "直接设激光原点 (x,y) 并解除挂靠;坐标世界层钳 ±LASER_COORD_MAX",
        param_names: &["lz", "x", "y"],
    },
    Builtin {
        name: "lz_cancel",
        syscall: syscall::SYS_LASER_CANCEL,
        is_op: false,
        params: &[Val(Int)],
        ret: None,
    ...
    Builtin {
        name: "lz_alive",
        syscall: syscall::SYS_LASER_ALIVE,
        is_op: false,
        params: &[Val(Int)],
        ret: Some(Int),
