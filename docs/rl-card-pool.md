...
1. **段落必须以结束事件收尾。** 用一只 boss 敌 + `phase_begin` / `spell_begin` 带时限，时限到自动发事件。
   **boss 要 `set_invuln(65535)`**：SHOT 恒按，不无敌的话段长会随模型站位变，这张卡的难度就不稳定了。
   ...
   > **小怪也要无敌**（2026-09-20 拍板；TH06 转写池的 166 个敌 sub 已全部如此）。理由不是省事：
   > 躲弹模型的职责只有「到指挥点 + 别死」，**火力站位是上层意图模型的职责**。小怪若能打死且
   > 收益明显，躲弹模型会自己长出「往敌群靠能减压」的议程，在指挥点与敌群不一致时偷偷违抗
   > 指令 —— 目标污染。无敌把两层的职责边界焊进训练分布里；训练压力因此是**上界**，部署时真实
   > 压力更低，误差方向安全。已知代价：难度画像会高估道中卡、模型学不到「再撑 2 秒这波就没了」。
   >
   > **真实关卡（给人玩的那批）不适用本条** —— 那里小怪本来就该能打死。届时要先定两个数：
   > 自机火力档位（现在 RL env 用 `Loadout{power:0}` = 单发、每 4 帧 1 伤 = 0.25 伤害/帧）
   > 与小怪血量（照抄 TH06 的 50 血没有意义：那是 ZUN 的火力口径，按我们的要 200 帧才打死一只）。
...
7. **激光可用**（2026-09-25 起）。引擎已有激光池（`laser()` / `lz_*`，手册第 9 篇
   `docs/ecl-lang/9-lasers.md`）——原作的激光段**直接转写**，不再拿密排弹链近似。
   TH06 转写有两条换算：**宽度减半**（原作画面是判定 2 倍，写 `width = 原作值/2`）；
   循环里每帧转一次的写法换成 `lz_omega`（每 N 帧转 `a` → `lz_omega(lz, (a as int / N) as angle)`）。
   完整口径见 spec `2026-09-25-laser-pool-design` §9.1。`laser_approx` 只留给确有必要近似的残余情形。
