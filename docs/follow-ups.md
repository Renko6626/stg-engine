...
9. ~~**部署侧复刻胶水**~~ **大半已销（2026-09-18，th06nc onnx 后端落地）**：改成**特征化连同网络一起导进 ONNX 图**（renkolab spec `2026-09-18-th06nc-onnx-policy-design.md` D2），所以 C 侧**不需要**复刻 topk / 密度图 / 归一化 —— 逐位一致由「两边是同一张图」保证。C 侧只剩「把 `ap_world_t` 填成七个原始表数组」（契约仓 `c/sa_model.c`，约 110 行）。**余项**：唯一必须由 C 侧主动做的分布对齐是**按训练包络过滤弹**（`x ∈ [-256,256]`、`y ∈ [-64,512]`，即本仓 `OOB_MARGIN = 64`）—— 密度图对格外坐标是钳位不是丢弃，不过滤的话场外弹会全堆进边缘格。改 `OOB_MARGIN` 要同步 `sa_model.h` 的 `SA_MODEL_ENV_*`。
   **补（2026-09-17）出生中的弹** —— **已销（2026-09-18）：不需要做**。核过一手：th06nc 的 `state 2/3/4` 是三档出现动画、各有自己的速度分母（renkolab `engine/bullet/th06nc/01` §2），期间**不参与被弹判定**，而抽取器对它们填 `collidable = 0`，与本仓 `encode.rs` 的 `flags` bit0 `= (delay == 0)` 同构 —— 掩码与密度图都只认这一位，所以这些弹对模型根本不存在，不存在「按全速外推」的问题。**残余风险是反方向的**：真游戏里弹「晚 10–32 帧才对模型现身」，预警时间比训练时短（登记在那份 spec §9 第 3 条）。原文如下，留档：
   **〔原记录〕**：训练卡池是 TH06 转写，但**有意不模拟**原作的出生特效（ECL `flags` 2/4/8：出生后 10/16/32 帧
...
11. ~~**激光读口**~~ **已完成（2026-09-25）**：`lz_x / lz_y / lz_angle / lz_near / lz_far`（syscall 810–814，只读、不计数，
    失效句柄押 0）。用途：TH06 的 `ex_ins_call(12)` / `ex_ins_call(14)`（在激光上 / 沿激光发弹），以及任何挂靠或 `lz_omega`
    的激光——脚本自己不知道它当前的几何。手册第 9 篇「读几何」。
...
### F26. RL 训练吞吐的引擎侧待办——详单在 `docs/rl-perf-roadmap.md`（RL 性能调研记档，2026-09-24）

训练仓在 A100 上分阶段计时后对本仓做了三路调研。已落地两件（`rl-v0.1.1`：VecEnv 分块、`tick_steps`
遇 END 即停）；**未做的六件**（RL 专用观测缓冲〔跨三仓，需先出 spec〕/ 无活跃 STEP 跳过 `tick_steps` /
ECL 加载时校验 / 道中卡每帧轮询 / CART_FX 极坐标惰性化 / reset 与 warmup 成本）连同数字、风险与一致性
方案都在那份文档里，本条只作索引。**拍板**：引擎未正式发布，改变校验和可以接受。
