...
### F12. demo 局在 Lunatic 下把**道具池**打满（`pool_full 104`）——`run` 逐池化之后才看得见（技术债刀，2026-09-03）

`cargo run -p stg-harness -- run godot/ecl/demo --frames 9000 --rank 3` 报
`pool_full 104`，逐池列出是 **道具 104**（不是弹、不是任务——这正是把 `run` 的
`pool_full` 从"一个总数"改成"逐池列表"之后第一件被照出来的事）。**先于本刀存在**：
在 `main` 上跑同一条命令得到一模一样的 104。

**多半不是内容 bug，是 harness 场景的性质**：`run` 不喂任何输入，自机全程杵在出生点、
**一个道具都不吃**；道具只能靠落到回收线外被清掉，于是 512 格的道具池在长跑里被垫满。
真人玩的时候自机会一路吃过去，压力完全不同。

**要判定它到底要不要管，得先分清两件事**：① 长跑 + 零拾取导致的池积压（无害，harness
场景性质）；② demo 的掉落量本身在 Lunatic 下就超预算（真问题，会在真人局里也丢道具）。
判决程序：给 `run` 加一个"自机沿固定轨迹移动"的合成输入档（或直接在真工程里打一局
Lunatic 看 HUD 的道具数），对比 `pool_full` 是否仍非零。

**触发点 = 内容平衡期**，或谁真的在 Lunatic 局里看到道具没掉出来。
在那之前它只是 `run` 输出里的一行警告——**但别把它当噪声无视**，它有可能是真的。
