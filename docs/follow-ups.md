...
**ZUN V2 对照增补**（Priw8 文档核对，2026-07-18）：⑤ **time label 语法糖**——ZUN 脚本
"时间轴+命令式"混合（`+N:` 满地），是弹幕编排手感核心；表层语言应支持 `+N:`/绝对时刻
标签，**编译期翻译成 WAIT 序列**（VM 保持纯命令式零复杂度）；⑥ **引擎状态变量化**——
ZUN 负 ID 寄存器（自机位/敌位/HP/帧龄）的作者手感值得吸收：表层把 `self_x` 等呈现为
变量、编译成 SYS 读（沙箱不变）；**但拒绝 ZUN 的"随机数变量"**（-10000 每引用即抽号
——RNG 消耗序藏进参数求值，审计噩梦，显式 `rand_range` 不动摇）；⑦ `jnz` 收编 op 表
v2 候补（ZUN jz/jnz 双条，我们跳非零现多绕三字——4x 空隙零成本）。
