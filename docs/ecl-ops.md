...
| 79 | `nearest_enemy`（小清洗刀 2026-07-31） | x,y | 离 `(x,y)` 最近的活敌的**池 index**；场上无敌（或全 `ENEMY_DYING`）→ **-1**。世界侧 `world::nearest_enemy` 自 M0-13 就有实现和测试、从没有 syscall 暴露过（死代码），本号只是给它通电，世界侧一行未改：候选 = 存活且非 dying、平方距离 i64 比较、并列取**低索引**（I4）、无距离上限。**P4-b**：押的是池 index，**generation 被丢弃** ⇒ 悬垂/槽复用不可辨——与 12 号 `enemy_hp` 完全同口径（两者本就配对用：本号拿号 → 12 号轮询）；空场返 -1 不计违约（合法世界状态非违约）、不 Fault。**owner 类别无限制**（关卡编排任务也该能查） |
| 80 | `enemy_x`（敌坐标读口刀 2026-07-31） | handle | 按敌号读 **x**（`Fx` raw）。补的是 79 号通电后暴露的断头路：脚本拿得到敌号却读不到坐标，"查最近的敌 → 朝它开火"算不出角度；数据本就在敌池里，缺的只是读口。`handle` 是**池 index**（直读、不比对 generation，同 12/79 号口径 ⇒ 悬垂/槽复用不可辨）。**P4-b 降级**：负句柄 / 越界 / 死槽 → **返 `0`**，不 Fault、**不计 `contract_viol`**（纯读族口径，照抄 12 号 `sys_enemy_hp` 的存活判据 `handle >= 0 && idx < CAP && is_alive(idx)`，逐字相同、只有降级值不同）；`ENEMY_DYING` 的敌**仍可读**（`is_alive` 是存活位，dying 只是 flag，槽活到相位 9 才回收）。**为什么降级值是 `0` 而不是哨兵**：12 号能用 `-1` 是因为 hp 天然非负，坐标没这个便利——`-1` 是完全合法的 `Fx` raw（≈ −0.0000153），没有哨兵位可用；取 `Fx::ZERO` 与 `self_pos` 对 STAGE owner 的第三分支同款。**代价**：「敌恰在原点」与「号无效」读起来一样，故探活归脚本——先 `enemy_hp(e) != -1` 再读坐标（写进 `docs/ecl-lang.md` 的惯例）。⚠️ 探活判据**不是** `enemy_hp(e) >= 0`：overkill 的敌 hp 是真实负值（`settle::kill_enemy` 只 `min(0)`，不抹平负血），`>= 0` 会把"刚被打穿、槽还在"的敌误判成无效。**owner 类别无限制**（关卡任务也该能查）。判别腿把敌放在 **x ≠ y** 的 (30, −70)（放 (5,5) 则两条派发臂写反完全不可辨），外加一条 `.ecl` 源码级 e2e 走完 `nearest_enemy → 探活 → 读坐标 → atan2 → fire` |
