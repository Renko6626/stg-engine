...
- 角度字面量必须带 `deg`/`bam` 后缀——裸 `90` 是 `int`，用在角度位**不会隐式转换**
  ...
  （`90 as angle` ≠ 90°）。
- **`wait(n)` 的 `n` 被静默截成低 16 位**：`wait(-1)` 等 **65535** 帧、`wait(70000)` 变成
  4464——没有任何诊断。上限 65535 帧（≈18 分钟），要等更久套循环。⚠️ **`wait(65536)`
  截断成 `wait(0)`，而 `wait(0)` 是"当这句不存在"的真 no-op**——搁在 `loop` 里就是死循环
  （被 `FAULT_BUDGET` 杀掉），不是"等 0 帧然后继续"。详见下方语句节。
...
`wait(n);`（n: int 帧，**等 n 帧**，**低 16 位截断**，见下）·
...
> ⚠️ **2026-08-01 之前不是这样**：旧实现的周期是 **n+1**（`wait(1)` 是"隔一帧跑"），
> 上面那段循环实际走 30×21 = **630** 帧，凡是自己记帧数的脚本一律偏 1/n。
> 这次修正 bump 了 `ENGINE_VER`（11 → 12），旧回放/存档拒载。若你手上有为旧语义
> 调过参的脚本，把补偿量去掉即可（当初写 `wait(19)` 凑 20 帧的，现在改回 `wait(20)`）。

**`wait(0)` = 当这句不存在**（不让出，同帧继续往下执行）。这样 `wait(delay)` 在 `delay`
算出 0 时行为自然，不必调用方特判。⚠️ **代价**：`wait(0)` 挡不住循环——

```text
loop { wait(0); }        // 真·死循环：烧穿单任务指令预算，Fault(3) FAULT_BUDGET
```

这和 `loop {}` 一个下场（**确定性的响亮失败**，不是挂死也不是 UB）。要"每帧跑一次"请写
`wait(1)`。

> ⚠️ **`wait(n)` 的 `n` 被截成 `u16`（取低 16 位），静默、无诊断**：任务的等待计数器是
> `u16`。所以 **`wait(-1)` = 等 **65535** 帧**（不是"立即继续"），`wait(70000)` 悄悄变成
> 4464，而 **`wait(65536)` 截断成 `wait(0)`** ⇒ 按上面的规则是**真 no-op**，写在 `loop`
> 里就是死循环——不是"等 0 帧"。语义有测试钉死（`ecl/vm.rs` 的
> `wait_truncates_to_low_16_bits`），**不是 bug、不会改**。
...
## 数学与查询（`atan2` / `dist` / `nearest_enemy` / `enemy_x` / `enemy_y` / `enemy_alive`）

前三个是引擎里早就有、脚本此前够不着的东西（小清洗刀 2026-07-31 通电）；中间两个是敌坐标
读口（敌坐标读口刀 2026-07-31），数据本就在敌池里躺着；`enemy_alive` 是探活专用口
（探活读口刀 2026-07-31）。六个都零新机制。
