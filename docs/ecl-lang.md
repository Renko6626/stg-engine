...
`.ecl` 是本引擎的弹幕脚本语言。**这份文件是写 `.ecl` 的唯一权威**——语法、类型、内建函数、
`$` 引擎变量、xformdef、错误格式都在这里。别凭对 ZUN ECL 或别的弹幕 DSL 的记忆脑补语法。

改完跑一句 `cargo run -p stg-harness -- check <file.ecl|目录>`（行列报错；目录 = 多文件整局，
见「多文件」节）。下方「内建函数」是从 `builtins.rs` 生成的段，**签名以它为准**；要改签名去改
`builtins.rs` 再跑 `gen-ecl-meta`，手改生成段会被 `cargo test` 的防漂移断言打回。
...
不像 Rust 的地方也不多：没有 `struct`/`enum`/泛型/trait，sub 无返回值，禁递归，
局部变量的槽由编译器静态分配。完整语句表见「语句」节。
...
读到中途值。它们让"相对运动"不需要专门的 Rel 版动词，见「敌人运动」节的 composed 写法。
...
另起变量镜像它**，也别自己给系统段槽号起名——引擎已经注入了具名常量，见「引擎常量」节。
...
成立——主任务跑完这只敌就退场，见下一节。所以 `stage` 侧应该轮询 `enemy_hp` 而不是去猜某个
sub 有没有退出。`enemy_hp(handle) -> int` 是 STAGE 层等 boss/敌死的标准写法：死亡、悬垂、
...
下一节的规则**这只 boss 会在 60 帧后自己退场**，等待循环随之结束。它按
...
（见「类型」节），走 `wrapping_mul`——**`$self_hp` 超过 32767 时这个乘法在 i32 里回绕**
...
违约，不 Fault、不钳位。写错了敌就是不动，别指望它退化成线性。
...
绑卡 boss 尤其要留意：`die()` 把 hp 压到 0（压过血线下钳，见「符卡」节），D9 自燃则完全
...
槽是每任务私有的四个，编号 `0..=3`。写 `4` 或负数 = 整条调用 no-op + 违约计数，不 Fault，
...
（`(32768 / n) as angle` 里的 `as angle` 是位穿透 cast，不是"转成度"，见「类型」节。这里要
...
否定）：多文件是"合并后单管线编译"，`const` 天然跨文件可见，见「多文件」节。仍然没有的
