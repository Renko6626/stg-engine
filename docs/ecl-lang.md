...
退场是**静默的**：不掉道具、不加分、不发 `EVT_ENEMY_DIED`、不发死亡特效请求——脚本跑完是
"退场"不是"被击破"，掉落与记分只属于打死它的那条路径。
...
`spell_begin` 的模式任务）调它们一律 **Fault(0)**、任务当场被杀——和 `move_to`/`spell_begin`
同一条门禁。
...
| 路径 | 掉落 | 加分（敌的 `score`） | 死亡事件 / 死亡特效请求 | 触发方式 |
|---|---|---|---|---|
| 被自机打死 | ✔ | ✔ | ✔ | hp ≤ 0（自机弹或消弹区伤害） |
| `die()` | ✔ | ✔ | ✔ | 脚本显式调用 |
| 主任务跑完（D9） | ✘ | ✘ | ✘ | 主任务自然 `return` / 跑到末尾 |
