...
| 1 | `SET_SPEED` | speed (Fx raw) | — | 1 | 置速率，回填 `vx/vy` | ✅ |
| 2 | `ADD_SPEED` | Δspeed (Fx raw) | — | 1 | 速率增量（可负） | ✅ |
| 3 | `SET_ANGLE` | angle (BAM) | — | 1 | 置朝向，回填 | ✅ |
| 4 | `TURN` | Δangle (BAM，可负) | — | 1 | 相对转向 | ✅ |
| 5 | `AIM_PLAYER` | Δangle (BAM) | — | 1 | 瞄最近可瞄自机 + 偏移（可瞄 = 非 ABSENT 非 GAMEOVER；无可瞄自机 → 静默 no-op） | ✅ |
| 6 | `SET_SPRITE` | sprite id | — | 1 | 换贴图 | ✅ |
| 7 | `SET_LIFE` | 寿命帧 | — | 1 | 重设寿命（"到时自爆"惯用法） | ✅ |
| 8 | `SET_ANG_VEL` | ω (BAM/帧, i16 语义) | — | 1 | 开 `POLAR_FX`（清 CART）——旋转弹 | ✅ |
| 9 | `SET_ACCEL` | a (Fx raw/帧²) | — | 1 | 沿向加速，开 `POLAR_FX`（清 CART） | ✅ |
| 10 | `SET_GRAVITY` | ax (Fx raw/帧²) | ay (Fx raw/帧²) | 1 | 笛卡尔加速，开 `CART_FX`（清 POLAR）——重力/漂移 | ✅ |
| 11 | `STOP_FX` | — | — | 1 | 清两模式位（连续效果全停） | ✅ |
| 12 | `LOOP` | target_slot (0..16) | count | 1 | 游标跳回 target；count 语义见下 | ✅ |
| 13 | `WAIT_SIGNAL` | ch (0..8) | — | 1 | 停驻等信号脉冲（边沿触发，"全场齐转向"） | 🚧 M0-11b |
| 14 | `BOUNCE_ARM` | walls 掩码（低 4 位：左/右/上/下） | n (≤3) | 1 | 反弹待命（场界折返镜像） | 🚧 M0-11b |
| 15 | `STEP_SPEED` | target (Fx raw) | frames (低16) \| easing id (高8) | **2** | 限时缓动到目标速率 | 🚧 M0-11b |
| 16 | `STEP_ANGLE` | target (BAM) | 同上 | **2** | 限时缓动到目标角（最短弧） | 🚧 M0-11b |
| 17 | `SPAWN_PATTERN` | pattern_id | Δangle | 1 | 按图样描述符表发一批子弹 | 📋 预留（随图样表另立一刀） |
...
- **模式位与游标正交**：连续效果（8/9/10）是持久开关，游标只负责在排程点开/关/改参。
  "之字 + 加速" = 循环里 TURN + 开局一次 SET_ACCEL——单游标天然并发。
  8/9 互相兼容（同属 POLAR），与 10 互斥（置一清另一）。
