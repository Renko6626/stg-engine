...
- **wait**：发射本 op 后**恰等 `wait` 帧**再执行下一槽；`wait=0` 同帧连发。
  ⚠️ **表层记号 `@N op(...)` 里的 `N` 是这条 op 自己的 wait**——读起来像"到第 N 帧才做这条"
  的时间标签，实际是**后置延迟**（先执行、再等）。详见
  [`docs/ecl-lang/4-bullets.md`](ecl-lang/4-bullets.md)「`@N` 是后置延迟，不是时间标签」节。
  ⚠️ **段的时钟从弹的创建当帧起跑，而 `sh_task` 派的任务出生当帧不跑**——拿 `set_life` 自毁
  配 `sh_task` 开火做分裂弹时两条腿天生错开 1 帧，静默失败（子弹一颗不出、无任何诊断信号）。
  详见同一篇的「`sh_task` 与 xformdef 天生错开 1 帧」节。
...
- **LOOP 回跳会收缩"已发射区间"`[0, xform_next)`**（B10，看起来像 bug 的正确行为）：
  活跃 STEP 的推进与 `BOUNCE_ARM` 的 walls 读取**都只扫这个区间**，而回跳把右界改小了。
  于是跳过点之后的那些 op 在回跳当帧**暂时不存在**——活跃 STEP 冻结一帧（直到游标重新
  走过它、重武装），`BOUNCE_ARM` 的 walls 读 0、反弹当帧失效（`flags` 里的**剩余次数
  不受影响**，失效的只是"读得到哪面墙"）。确定且跨平台一致，是 spec 的字面执行；
  **别把两处扫描的右界"顺手"改成扫全段**——walls 影响弹的轨迹，直接进校验和。
  判别式测试：`world/transform.rs::loop_jump_shrinks_fired_region_so_bounce_walls_read_zero_for_one_frame`。
