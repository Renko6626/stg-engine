...
> `docs/bench-baseline.md` 末节。
> **已落地**（`ENGINE_VER` 23 / `stg_rl` 0.2.0，引擎第二刀，2026-09-24）：§2 弹 `BULLET_STEP_LIVE`
> 跳过位、§3 ECL 加载时校验、§5 `CART_FX` 极坐标惰性化；另外敌人池新增 `dx/dy` 一等速度字段，
> 训练侧不再需要按 id 差分（不在本文任何一节里，是本刀新增范围，详见
> `docs/superpowers/specs/2026-09-24-engine-rl-round2-design.md` §6）。实测（`docs/bench-baseline.md` 末节）：
> 三项只打中变换密集卡与重力卡（逐卡最多 ×1.86），卡池整体 `env_step` 约 −4%；A100 端到端稳态 89k → 94–96k，
> 大头来自训练侧删掉按 id 差分。**剩下最值钱的是 §1**（h2d 0.36 s，其中敌人整表拷贝因行变宽还涨了 0.014 s）。
> 本文只记**还没做**的。
...
| 训练侧阶段 | 秒/更新 | 与本仓的关系 |
|---|---:|---|
| `env_step` | 约 0.55 → **0.29** | 本仓 `VecEnv::step`；`rl-v0.1.1` 分块修复后 Magnus 实测减少 47%，稳态吞吐 +22~25%（训练仓 Job `96202eaedd57a7f4`） |
| PPO 更新 | 约 0.55 | 与本仓无关 |
...
## 2. 没有活跃 STEP 的弹整段跳过 `tick_steps` ——**已落地（ENGINE_VER 23 / stg_rl 0.2.0）**
...
## 3. ECL 虚拟机：加载时一次性校验 ——**已落地（ENGINE_VER 23 / stg_rl 0.2.0）**
...
但属于语义变化，走评审。**落地记录**：取指与操作数越界检查**保留在运行时**没有一并去掉——加载时
校验挡的是「镜像整体合法」（op 已实现、跳转目标合法），运行时那道检查挡的是 `FAULT_BUDGET`/
坏参数这类只有跑起来才暴露的情况，两者不是互斥关系，落地时没有按最初设想把运行时检查全部搬空。
...
## 5. CART_FX 极坐标惰性化 ——**已落地（ENGINE_VER 23 / stg_rl 0.2.0）**
