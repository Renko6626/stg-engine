...
## 9. 修订记录（实现期实测，2026-07-31）

三条本 spec 写错或写反的事实，逐条留痕。**不改上面的正文**（正文是当时的判断，改掉就看不出
错在哪了），以本节为准。

> **修订记录一（Task 2 期发现）：§3 那句「调用方全符号引用，`rg` 确认无任何硬编码数字」不对。**
> `crates/stg-ecl-compiler/src/lang/mod.rs` 的
> `fire_and_batch_bytecode_is_byte_for_byte_unchanged` 就硬编码了 `OP_SYS` 的操作数——期望
> 字节流是个裸 `[u32]` 数组，`60, 200` 那两位里的 `200` 正是 `SYS_CREATE_BULLET`。
> spec 期的扫描正则只找 `syscall::SYS_*` 的符号引用与形如 `sys(\d+)` 的调用，**漏了裸数组里的
> 裸数字**。
>
> **而且这是一处「好的硬编码」，不该改成符号**：那条测试的价值就在于把编译产物**逐字节钉死**
> ——写成 `SYS_CREATE_BULLET` 就变成了同义反复，改错号也照绿。Task 1 已按预期同步更新了这两
> 个数组（`20`→`200`、`21`→`201`）并在断言消息里注明「除 syscall 号重排换的操作数外」。
>
> **含义（写给下一次重排号表的人）**：那条钉死测试**必然**要跟着改，这是**预期，不是回归**。
> 「重排不动任何测试」这条口径只对**按名引用**的 74 条行为测试成立，对字节钉死测试不成立。

> **修订记录二（Task 2 期发现）：§3 的「syscall 全部推到 100 以上后，两个号空间永久错开」
> 对 `0xx` 族不成立。**
> `0xx`（`$` 引擎变量）取值 000–032，而 op 是 `u8`、现最大 60（`OP_SYS`）——**整族 12 条都落在
> op 号域内**（`self_x`=20=`OP_ADD`、`player_x`=10=`OP_PUSHI`、`frame`=0=`OP_END` …）。
> 故 §3 顺带许诺的「消掉 `lang/mod.rs` 那条假阳性注释」**没有兑现**：该注释保留，只把例子从
> `SYS_CREATE_BULLET == 20` 换成 `SYS_SELF_X == 20`。
>
> 何况那个坑本来就不只是 syscall 的事——`PUSHI 20` / `POPL 20` / `JMP 20` 的操作数字同样长得
> 像 `OP_ADD`，**裸扫字节流找 opcode 结构上就不可靠**，与号表怎么排无关。`opcodes_of` 这个
> 按 `ARITY` 走流的助手不会因为任何一次重排而退休。

> **修订记录三（Task 2 期实测）：§5 担心的稀疏 `match` 退化没有发生；§7 预期的「金向量流会变」
> 也没有发生。**
>
> **§5（派发代价）**：反汇编 release 产物看 `ecl::syscall::dispatch`——
> 重排**前**是 `cmp $0x5a` + **一张** 91 项跳转表；重排**后**是 `cmp $0x294` + **两张**跳转表
> （0..=660 一张、661..=741 一张），共 742 项。**没有退化成二分查找**，仍是 O(1) 间接跳转，
> 只多一次范围比较（+13 条指令、+1 条条件分支、跳转表 rodata 364 B → 2968 B）。
> 实测：仓库自带的 `bench` **对本刀结构性失明**——它的四类场景全部传 `EclImage::empty()`，
> 一条 `OP_SYS` 都不执行，前后差异纯是噪声。另跑一个 ~100% 打在 syscall 派发上的合成负载
> （60 个任务 × 220 条 syscall/帧，涵盖 `0xx`/`1xx`/`7xx` 三段号域）：
> 133.24–133.41 µs/帧（前） vs 133.58–134.26 µs/帧（后），**+0.3~0.6%，≈ 0.04 ns/syscall**。
> **结论：不改派发实现**（按族 `n / 100` 分派的备选方案不启用）。
>
> **§7（金向量）**：`golden` 两段场景 1200 帧校验和流**逐字节相同**（md5 一致）。原因是
> `EclImage` 是 `step()` 的**静态入参**、不是 `World` 字段，校验和只哈希 `World`；而号只
> 换了 `OP_SYS` 的**操作数值**、没换字流长度，`Task.ip` 也一位没动。
> 所以本刀连"金向量得重新生成"都不需要——但这**不构成**行为不变的证明（CLAUDE.md 已写死金
> 向量闸门抓不了行为回归），行为不变的证据仍是 74 条按名引用的行为测试 + `.ecl` e2e + 两个冒烟。
