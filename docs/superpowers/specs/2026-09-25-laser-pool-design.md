...
- `ex_ins_call(12)`（第 4 关「在激光上发弹」）与 `ex_ins_call(14)`（Extra「沿激光铺弹」）都要读激光几何：
  引擎已加只读口 `lz_x / lz_y / lz_angle / lz_near / lz_far`（syscall 810–814，§12 修订），两者都**可以转**，直接读当前值，
  不必在脚本里另维护一份几何副本。
...
12. **激光几何读口**（2026-09-25，§5 增补）：`lz_x / lz_y / lz_angle / lz_near / lz_far`（810–814），只读、不计数、失效句柄返回 0。`lz_start` 已是写口，故读近端 / 远端偏移叫 `lz_near` / `lz_far`。
