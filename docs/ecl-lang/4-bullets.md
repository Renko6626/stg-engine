...
| 路径 | 什么时候解析 | 从哪个点算 | 瞄谁 |
|---|---|---|---|
| `aim_player()` | 调用那一刻 | owner 自己的位置（敌 → 敌中心，弹 → 弹自己，无 owner 的关卡脚本 → 世界原点） | 恒 `players[0]`，不查存活 |
| `sh_aim` + `sh_fire` | **`sh_fire` 那一刻** | **出弹口** = owner 位置 + `sh_offset` + 极坐标偏移 | 恒 `players[0]`，不查存活 |
| xformdef 里的 `aim_player` op | 变换游标走到那条 op 的那一帧（可以在飞行途中） | 弹自己**当时**的位置 | 最近的**可瞄**自机 |
| 弹 setter `aim_at_player(h, off)` | 调用那一刻 | 弹自己的位置 | 最近的**可瞄**自机 |

四条**全都是"事情发生的那一刻"才解析**，没有一条在声明时定死。但右边两列有两处不对称，
自己写瞄准逻辑时值得知道：

- **基点**：前两条从射手算，后两条从弹自己算（弹已经飞出去了，从它当前位置瞄才对）。
- **瞄谁**：前两条恒取 `players[0]`、不查存活；后两条取**最近的可瞄自机**（缺席或已 game
  over 的跳过），一个可瞄的都没有时整条 **no-op**——弹保持原角度，不报错也不计数。本仓
  `MAX_PLAYERS = 2`，所以这条差别在单人局里只在"自机已 game over"时看得出来。
