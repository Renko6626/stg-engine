...
原点 `ox/oy`、角度（BAM 原值 + 度数）、`start/end`、`width`（判定宽度）、`speed`、`omega`、
`state`（0 预警 / 1 生效 / 2 收缩）、`timer`（本态已推进的步数）、三段时长和颜色。「峰值」「末帧」两行末尾也带激光条数，
峰值帧就是第一条激光出生的那一步。核「预警几帧、第几帧扫到哪个角度」就看这张表。
