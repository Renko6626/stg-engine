...
- **转换是 1:1，而道具池（1024）比弹池（8192）窄 8 倍**——所以"一次消掉的弹比 1024 多"时，
  多出来的那些**星星生不出来**：逐颗计 `diag.pool_full[POOL_ITEM]`、不 Fault、不报错，
  弹照消不误，你只是少拿了那部分分。**这是已知设计边界，不是引擎 bug**，而且它在真内容里
  出过：demo 局收卡那一帧场上 626 颗弹，道具池还是 512 的时候**四个难度档全部溢出**
  （Easy 3 / Normal 37 / Hard 67 / Lunatic 104 颗），F12 把池抬到 1024 才盖住。写"一次清
  非常多弹"的编排时（尤其是收卡消弹叠上一屏弹幕）心里有个数。
- **不给无敌帧**。它只消弹，不碰自机状态——刚清完弹下一帧照样能被新弹打死。游戏本体机体
  （`Kit::Chronos`，机体 0）没有 bomb（玩法刀 2026-09-14 退役）：X 是**停止**——时停 180 帧，冻结期间自机判定圆碰到的冻弹被抹掉
  （碰撞行 8，每颗 +10 分），库存就是 `add_bombs(n)` 管的那份（上限 5）。机体 1（经典训练机体）的
  X 是经典 bomb，库存同一份。脚本侧能做的只有
  `add_bombs(n)` 发库存、`clear_bullets()` 清场。
...
写 `freeze_left[1]`（ECL 演出档，与自机技能的时停用的是**另一个槽** `[0]`），冻的是 A+B
两组：自机不能移动、不能发新弹，已经在场上的自机弹也停住不动；**敌方照常行动**——敌人的
移动/发弹/协程都不受影响，这是它的用法定位：脚本想演一段"自机被定住、敌方继续动作"的
演出（比如 boss 开场宣言、中段过场），而不是全场暂停。

**碰撞判定不冻**——冻的只是 A/B 两组的行为写入，相位 6 体碰照常判定，场上已有的敌弹会
继续命中定住的自机。这正是它作为"演出"而非"安全时停"的威胁所在：脚本若想在定住期间
保命，得自己另外处理（比如同时清弹），`time_stop_player` 本身不给无敌。

- `time_stop_player(90)` 定住 90 帧。
- `time_stop_player(0)` 是**立即解除**——天然的取消 API，不需要另开一个 `cancel` syscall。
- **重入取覆盖**：连续两次调用，后一次的 `frames` 直接盖掉前一次的剩余帧数（不取最大、
  不叠加）。脚本是权威、覆盖可预测——想续时长自己在脚本里读剩余帧数再决定传什么。
- `frames` 收窄到 `u16`（`0..=65535`），负值或 `>65535` 越界时**整条 no-op**（不钳位）+
  计 `diag.contract_viol` + `last_status=BAD_ARGS`（D19 判例：与运动动词族 `dur`/`easing`
  同一条收窄口径），不 Fault。
